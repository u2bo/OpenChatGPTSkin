...
- 手动 `workflow_dispatch` 构建并上传 Windows 和两套 macOS 测试产物及诊断，不创建 Tag 或 Release；版本 Tag 在 Windows 和两套 macOS 门槛全部通过后才发布组合 Release。
...
- Manual `workflow_dispatch` runs build and upload Windows plus both macOS test artifacts and diagnostics; they never create a tag or Release. A version tag publishes the combined Release only after Windows and both macOS gates pass.
