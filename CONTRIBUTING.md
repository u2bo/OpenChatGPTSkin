...
- 不手工拼装或上传资产。Windows ZIP、Setup 和 `checksums.txt` 必须由 `.github/workflows/release.yml` 从同一 staging 构建，并通过最终归档与安装生命周期验收。
- macOS 构建在真实 Mac、签名、公证和 Gatekeeper 清单完成前只保留为 CI contract artifact，不得附加到公开 Release。
...
- Do not assemble or upload assets manually. The Windows ZIP, Setup, and `checksums.txt` must be built by `.github/workflows/release.yml` from one staging payload and pass final-archive plus installer-lifecycle acceptance.
- macOS builds remain CI contract artifacts until the real-Mac, signing, notarization, and Gatekeeper checklist is complete; do not attach them to a public Release.
