...
> [!IMPORTANT]
> `v0.1.0-alpha.1` 是 **Windows 开发者预览版**，提供 Windows x64 便携 ZIP 与用户级 Setup；两者都内置 Node.js，无需安装 Git 或开发依赖。macOS 当前仍是源码开发者预览，不附加到公开 Release，等待真实 Mac、签名和 Gatekeeper 验收。应用或恢复主题前，请保存工作并**完全退出普通 Codex**。OpenChatGPTSkin 只管理自己启动的 Codex 实例，不会强制结束已有 Codex，也不会修改 `WindowsApps`、`Codex.app`、`app.asar`、账号或 API 配置。
...
| macOS Runtime 启动、切换、暂停、恢复 | 源码预览，实机验收待完成 |
...
覆盖安装新版 Setup 或替换便携目录只更新程序文件，不迁移或覆盖 `%LOCALAPPDATA%\OpenChatGPTSkin`。卸载程序默认保留个人主题、草稿、版本和 Runtime 状态；仅在非静默卸载时明确选择“同时删除个人数据”并确认不可恢复提示，才会删除该数据目录。
...
2. Setup 用户从开始菜单启动；便携版双击 `OpenChatGPTSkin.cmd`；源码用户运行：
...
OpenChatGPTSkin 是独立的本地 Theme Studio 与 Desktop Runtime，不是 Codex 插件市场插件。Windows 用户应使用 GitHub Release 中的 Setup 或便携 ZIP；当前不会修改 Codex 安装包，也不会出现在 Codex 的插件页面。
