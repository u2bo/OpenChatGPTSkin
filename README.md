...
下面三张完整概念图展示 OpenChatGPTSkin 在人物、动漫和高能科幻方向上的可定制空间。图片保持原始宽高比，不做裁切。

> [!NOTE]
> 这些图片用于视觉概念与能力展示；其中“三上悠亚·星光粉”已作为授权混合素材主题实现，其余图片仍不是内置主题。概念图本身不随安装包提供，也不代表与相关人物、作品或权利人的官方合作；公开使用或再分发仍需遵守各主题独立的授权说明。

### 三上悠亚粉雾主题概念

<img src="docs/assets/concepts/yua-mikami.png" width="100%" alt="三上悠亚粉雾 OpenChatGPTSkin 完整主题概念图">
...
> 当前 Git 源码包含第五主题；已发布的 `v0.1.0` 安装包仍是四主题版本，第五主题会随下一次版本发布进入安装包。
...
柔粉霓虹、星光和授权人物背景构成的深色沉浸主题。它使用 Theme Schema v4 的中英文动态欢迎语、真实项目名插值、四个独立建议图标、用户头像及四个不可交互视觉图层；展示字体采用轻量的 `Arial` 与系统中文回退，不内置超限字体。

![三上悠亚·星光粉主题概念](docs/assets/concepts/yua-mikami.png)
...
1. 在 [GitHub Releases](https://github.com/u2bo/OpenChatGPTSkin/releases) 下载 `OpenChatGPTSkin_0.1.0_windows_x64_Setup.exe` 和 `checksums.txt`。
...
下载 `OpenChatGPTSkin_0.1.0_windows_x64.zip`，校验后解压到可写且稳定的目录，双击 `OpenChatGPTSkin.cmd`。便携版不会注册安装信息，也不依赖全局 Node.js 或 Git；个人主题仍写入 `%LOCALAPPDATA%\OpenChatGPTSkin`，不会写入程序目录。

### macOS DMG（未签名开发者预览）

1. Apple Silicon（M 系列）下载 `OpenChatGPTSkin_0.1.0_macos_arm64.dmg`；Intel Mac 下载 `OpenChatGPTSkin_0.1.0_macos_x64.dmg`。Intel x64 兼容性取决于官方 Codex 是否提供对应架构版本，目前尚未完成实机验证。
...
开发者还可以下载同架构的 `OpenChatGPTSkin_0.1.0_macos_arm64.tar.gz` 或 `OpenChatGPTSkin_0.1.0_macos_x64.tar.gz`。普通用户优先使用 DMG。
