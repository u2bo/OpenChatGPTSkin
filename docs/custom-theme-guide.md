...
两种方式产生相同的 Theme Schema v2 数据，可以相互导入导出。推荐先用 Theme Studio 找到满意效果，再导出 `.ocskin`；需要批量或自动化时再使用 AI 封装。
...
Theme Schema v2 主题和 .ocskin 文件，不要修改 Runtime、Theme Studio 或其他业务代码。
...
5. 使用 Theme Schema v2 的完整语义颜色；主文字、输入文字和代码文字必须可读，
