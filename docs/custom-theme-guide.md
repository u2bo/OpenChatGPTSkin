...
> [!IMPORTANT]
> Windows x64 用户可使用 GitHub Release 的 Setup 或便携 ZIP；macOS 用户可使用对应架构的未签名 DMG，Windows/macOS 开发者也可从源码运行。应用主题前必须完全退出普通 ChatGPT Desktop。主题只允许数据和本地素材，不能包含任意 JavaScript、HTML、CSS、可执行文件、网络素材 URL 或自定义 DOM 选择器。macOS 的真实应用视觉验收仍需按 [macOS Runtime 说明](runtime-macos.md) 在 Mac 上完成，不能把包验收当作真实 ChatGPT UI 已通过的证据。
...
两种方式产生相同的 Theme Schema v4 数据，可以相互导入导出。推荐先用 Theme Studio 找到满意效果，再导出 `.ocskin`；需要批量或自动化时再使用 AI 封装。
...
| 字体 | UI/代码/首页展示字体族、字号、缩放、字重、行高、字距和本地 WOFF2 字体 |
| 装饰 | 粒子、丝带、蝴蝶、拍立得、徽章、闪光和本地图片装饰 |
| 首页 | 中英文 1–3 行动态欢迎语、真实 `{projectName}` 插值、位置、宽度、对齐和原生图标显隐 |
| 界面素材 | 账户头像、四个建议卡片图标、最多 12 个项目图标及各自显示尺寸 |
| 组合图层 | 最多 24 个前景/装饰图层，可指定目标表面、锚点、坐标、宽度、透明度和旋转 |
| 布局 | 允许调整的模块顺序、显隐、尺寸、对齐、间距、侧边栏密度、输入区宽度和卡片列数 |
| 信息 | 主题 ID、名称、描述、版本、作者和权利元数据 |

项目选择、侧边栏、顶部栏、输入区和内容层等受保护区域必须保持可见。项目选择组件始终使用 ChatGPT 官方大小、位置和层级，只适配主题颜色；Theme Studio 不接受任意 CSS 或 DOM 选择器。Schema v4 的组合图层只能落在 `viewport`、`main`、`home-hero` 或 `suggestions` 安全表面，始终不接收鼠标和键盘事件。
...
- 可选人物前景、透明装饰图、账户头像、四个建议卡片图标、项目图标和 WOFF2 字体；
- 主题名称、英文 ID（如 `my-forest-theme`）、作者、版本；
- 浅色/深色倾向、主要颜色、主体焦点和文字安全区；
- 可选中英文欢迎语；需要真实项目名时仅使用 `{projectName}`；
- 可选组合图层清单：每层使用哪个素材、显示在哪个安全表面及其位置；
- 每个素材的来源与授权。
...
Theme Schema v4 主题和 .ocskin 文件，不要修改 Runtime、Theme Studio 或其他业务代码。
...
- 中文欢迎语：<1–3 行纯文本，可使用 {projectName}；留空则使用 ChatGPT 原生欢迎语>
- 英文欢迎语：<1–3 行纯文本，可使用 {projectName}；留空则使用 ChatGPT 原生欢迎语>
- 欢迎语布局：<锚点、水平/垂直位置、宽度、对齐、是否隐藏原生首页图标>
- 界面素材：<账户头像、建议卡片 1–4、项目图标 1–12；未提供的槽位保留官方视觉>
- 展示字体：<字体族或本地 WOFF2、字号、字重、行高、字距>
- 组合图层：<素材、viewport/main/home-hero/suggestions、锚点、坐标、宽度、透明度、旋转>
- 素材来源与授权：<来源、许可证、是否允许再分发>
- 是否仅本地使用：<true / false>

实施约束：
1. 完整阅读 docs/theme-format.md。以 themes/builtin/mountain-mist 作为基础结构参考，
   以 themes/builtin/yua-mikami-starlight/theme.json 作为完整 Schema v4 能力参考。
...
5. 使用 Theme Schema v4 的完整语义颜色；主文字、输入文字和代码文字必须可读，
   对比度不低于 Theme Studio 的保存门槛。
6. 根据背景主体设置 positionX/positionY、safeArea、overlay、brightness、surfaces，
   确保真实 ChatGPT 的菜单、设置、历史、任务、终端和输入框都能读取内容。
7. 受保护布局必须保持可见；project-picker 使用官方几何，不要试图改变其坐标或层级。
8. 仅在用户提供内容时配置 home.welcome；每个语言 1–3 行纯文本，唯一允许的占位符是
   {projectName}。缺少项目名或对应语言时必须回退到 ChatGPT 原生欢迎语。
9. 为头像、四个建议卡片和项目图标声明独立本地素材路径；未提供的槽位保持官方视觉，
   不得用隐藏 fallback 伪造素材。项目图标最多 12 个。
10. 如有展示字体，在 assets.fonts 声明 WOFF2，并让 displayFontAssetKey 引用真实 key；
    字体不可读或超出 Schema 范围时改用普通可读字体，不跳过校验。
11. 组合图层最多 24 个，必须引用 assets.portrait 或 assets.decorations 中已声明素材；
...
```text
生成一张原创 16:9、4K 桌面主题背景图，用于 ChatGPT Desktop UI。
风格：<自然 / 科幻 / 极简 / 复古 / 其他>。
...
│   ├── project-icon1.webp       # 可选，项目图标 1；最多 12 个
│   └── decorations/             # 可选
└── fonts/
    ├── ui.woff2                 # 可选
    ├── code.woff2               # 可选
    └── display.woff2            # 可选，首页展示字体
```
...
运行 `launch` 前必须完全退出普通 ChatGPT Desktop。
...
在“界面素材”中可以分别配置用户头像、四个建议卡片图片和四个可视化项目图标槽位。每个槽位支持上传/替换、清除或复用主题背景；清除后恢复 ChatGPT 官方视觉。头像会归一化为 `256×256 WebP`，建议卡片和项目图标会归一化为 `192×192 WebP`。Schema 可保存最多 12 个项目图标，Theme Studio 当前直接提供前四个槽位；项目列表会按顺序循环使用。修改只进入当前草稿，仍需点击“保存版本”才会产生新版本。

### 5. 欢迎语、字体、组合图层、装饰和布局

- 欢迎语：分别配置中英文 1–3 行纯文本，使用 `{projectName}` 注入真实项目名；未配置语言或缺少项目名时保留 ChatGPT 原生欢迎语；
- 展示排版：调整欢迎语锚点、位置、宽度和对齐；可选择隐藏原生首页图标；
- 字体：配置 UI/代码/展示字体、字号、缩放、字重、行高和展示字距；自带字体必须是 WOFF2；
- 视觉组合：上传最多 24 个非交互图层，设置目标安全表面、锚点、位置、宽度、透明度、旋转和必需标记；
- 装饰：最多 16 个非交互装饰层；
...
隔离预览和 Runtime 共用视觉模型，但真实 ChatGPT 版本仍可能改变 DOM。发现差异时记录应用版本和页面路径，修复适配器而不是加入任意 CSS。
...
1. 属性修改仅更新当前编辑和预览；
2. 点击“保存版本”才写入个人主题版本；
3. 已保存的精确 `{id, version}` 才能“应用到 ChatGPT”或导出；
4. 需要分享时从主题信息/版本区域导出 `.ocskin`。

应用前必须完全退出普通 ChatGPT Desktop。需要恢复时点击右上角“恢复原始皮肤”，随后正常退出受管理 ChatGPT 完成清理。
...
- [ ] 项目图标顺序、裁切和循环使用正确，清除后恢复官方图标；
- [ ] 中英文欢迎语、`{projectName}` 插值、无项目名回退和位置排版正确；
- [ ] 展示字体缺失时校验失败或明确恢复普通字体，不出现不可读文字；
- [ ] 每个组合图层引用已声明素材，位于安全表面且不拦截交互；
...
- [ ] 公共主题包含 attribution；
- [ ] `validate` 和 `pack` 成功；
- [ ] 以六个内置主题（`future-idol-cyan`、`rose-carpet-star`、`mountain-mist`、`glacier-aurora`、`hoshimiya-ichigo-shining-stage`、`yua-mikami-starlight`）为基线检查导入、预览、应用和恢复；
...
确认普通 ChatGPT Desktop 已正常完全退出，再重试。不要使用任务管理器强制结束受管理 ChatGPT。
...
然后通过 ChatGPT 菜单或系统托盘正常退出，完成恢复清理。
