...
3. Run `launch` and five `switch` commands. Check all six built-in themes across home, history, tasks, settings, plugins, menus, overlays, composers, sidebars, and terminal.
...
8. Record Codex/macOS versions, six-theme results, restore result, and sanitized screenshots. Do not record PIDs, ports, usernames, paths, command lines, project names, or chat content.
