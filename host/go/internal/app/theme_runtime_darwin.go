...
func newManagedThemeSession(ctx context.Context) (managedThemeSession, error) {
	...
	return &macOSThemeSession{connection: connection, root: launch.Root}, nil
}

func (session *macOSThemeSession) Apply(ctx context.Context, payload cdp.ThemePayload) error {
	return session.connection.ApplyTheme(ctx, payload)
}

func (session *macOSThemeSession) Restore(ctx context.Context) error {
	return session.connection.RestoreTheme(ctx)
}

func (session *macOSThemeSession) WaitForExit(ctx context.Context) error {
	return platform.WaitForManagedExit(ctx, session.root)
}

func (session *macOSThemeSession) Close() error { return session.connection.Close() }
